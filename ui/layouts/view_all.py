...
from data.store import get_all_students, compute_rankings
...
def init_ui(app):
    ...
    def refresh_table(students):
        ...
        for i, student in enumerate(students):
            CTkLabel(table_frame, text=student["student_id"]).grid(row=i+1, column=0, padx=5)
            CTkLabel(table_frame, text=student["name"]).grid(row=i+1, column=1, padx=5)
            CTkLabel(table_frame, text=f"{student['average']:.2f}").grid(row=i+1, column=2, padx=5)
